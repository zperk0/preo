...
}if("string"==typeof a){if(a.match(/(Z|([\+-]\d\d:?\d\d))$/))return new Date(a);var c=b(a);return new Date(a+c)}throw"Date param must be a valid ISO string"}}},{"../../node_modules/q/q":1}],33:[function(a,b){function c(){j=!1,g.length?i=g.concat(i):k=-1,i.length&&d()}function d(){if(!j){var a=setTimeout(c);j=!0;for(var b=i.length;b;){for(g=i,i=[];++k<b;)g[k].run();k=-1,b=i.length}g=null,j=!1,clearTimeout(a)}}function e(a,b){this.fun=a,this.array=b}function f(){}var g,h=b.exports={},i=[],j=!1,k=-1;h.nextTick=function(a){var b=new Array(arguments.length-1);if(arguments.length>1)for(var c=1;c<arguments.length;c++)b[c-1]=arguments[c];i.push(new e(a,b)),1!==i.length||j||setTimeout(d,0)},e.prototype.run=function(){this.fun.apply(null,this.array)},h.title="browser",h.browser=!0,h.env={},h.argv=[],h.version="",h.versions={},h.on=f,h.addListener=f,h.once=f,h.off=f,h.removeListener=f,h.removeAllListeners=f,h.emit=f,h.binding=function(){throw new Error("process.binding is not supported")},h.cwd=function(){return"/"},h.chdir=function(){throw new Error("process.chdir is not supported")},h.umask=function(){return 0}},{}]},{},[29]),angular.module("loaders",[]).service("$AjaxInterceptor",["$rootScope","$timeout",function(a,b){function c(){var a=document.getElementsByClassName(".loading-content");if(a.length>0){(new Spinner).spin(a[0])}}return{start:function(){++a.requests,c()},complete:function(){b(function(){a.requests>0&&--a.requests},100)},isRequesting:function(){return a.requests>0}}}]),function(){var a=null;try{a=angular.module("constants")}catch(b){a=angular.module("constants",[])}a.constant("FEATURE_STATUS",{INSTALLED:"INSTALLED",TRIAL:"TRIAL",UNINSTALLED:"UNINSTALLED"}),a.constant("FEATURES",{BOOKING:9})}(),function(){angular.module("bookingMenus",["ngRoute","constants","loaders","mm.foundation","gettext"]).config(["$routeProvider",function(a){a.when("/",{templateUrl:"/code/bookingMenus/partials/booking-menus.php"}).otherwise({redirectTo:"/"})}]).run(["$rootScope","gettextCatalog","LANG","$http","ACCOUNT_ID","FEATURE_STATUS","FEATURES",function(a,b,c,d,e,f,g){a.requests=0;var h="en_GB",i="en";switch(c){case"de":h="de_DE";break;case"fr":h="fr_FR";break;case"nb":h="nb_NO"}i=h.substr(0,h.indexOf("_")),"en_GB"!=h&&(b.currentLanguage=h,moment.locale(i)),d.get("/api/accounts/"+e+"/features/"+g.BOOKING).then(function(a){var b=!1;a&&a.data&&("INSTALLED"===a.data.status||"TRIAL"===a.data.status||"UNINSTALLED"===a.data.status)&&(b=!0),b||window.location.replace("/dashboard")})}])}(),function(a,b){b.module("bookingMenus").controller("MenusCtrl",["$scope","$rootScope","$timeout","$AjaxInterceptor","BookingMenusService","BookingSettingsService","gettextCatalog",function(b,c,d,e,f,g,h){function i(a){var b=a||h.getString("Sorry, but there's been an error processing your request.");noty({type:"error",layout:"topCenter",text:b})}function j(){c.requests=0,e.start(),g.getSettings().then(function(){f.getMenus().then(function(a){k.menusData=a||[];for(var b=0,c=k.menusData.length;c>b;b++)k.menusData[b].$promotions=k.menusData[b].promotions;e.complete()},function(){e.complete(),i()})},function(){var b=h.getString("Please, setup the group booking settings first.");e.complete(),i(b),d(function(){a.location.href="/bookingSettings"},2500)})}var k=this;k.createMenu=function(){a.location.href="/newGroupMenu"},k.duplicateItemMenu=function(b){k.menusData.push(b),a.location.href="/editmenu/"+b.id+"?duplicated=true"},k.removeItemMenu=function(a){c.safeApply(function(){k.menusData.splice(k.menusData.indexOf(a),1)})},c.safeApply=function(a){var b=this.$root.$$phase;"$apply"==b||"$digest"==b?a&&"function"==typeof a&&a():this.$apply(a)},d(j)}])}(window,angular),function(a,b){b.module("bookingMenus").service("BookingMenusService",["$http","ACCOUNT_ID",function(a,c){function d(a){for(var b=0;b<a.length;++b)void 0!==a[b]&&(a[b].insert=!0,a[b]["delete"]=!1,a[b].edit=!1)}var e={},f=c;return e.getMenus=function(){var a={accountId:f,type:"booking"};return Preoday.Menu.get(a)},e.save=function(c){var e=b.copy(c);e.id="menu1",e.type="BOOKING",d(e.sections);for(var f=0,g=e.sections.length;g>f;f++){d(e.sections[f].items);for(var h=0,i=e.sections[f].items.length;i>h;h++){var j=e.sections[f].items[h];d(j.modifiers),j.mdi=!0;for(var k=0,l=j.modifiers.length;l>k;k++){var m=j.modifiers[k];d(m.items),m.options=m.items}}}return a.post("/saveMenu",e)},e.remove=function(b){return a({url:"/deleteMenu",method:"POST",data:"menuID="+b,headers:{"Content-Type":"application/x-www-form-urlencoded"}})},e}])}(window,angular),function(a,b){b.module("bookingMenus").service("BookingSettingsService",["VENUE_ID",function(a){var b={},c=a;return b.getSettings=function(){return Preoday.Venue.getBookingSettings({venueId:c})},b}])}(window,angular),angular.module("bookingMenus").directive("menusItem",["$timeout","$q","$rootScope","gettextCatalog","BookingMenusService","$AjaxInterceptor",function(a,b,c,d,e,f){return{templateUrl:"/code/bookingMenus/js/directives/menus-item/menus-item.php",restrict:"A",replace:!0,scope:{menu:"=element",duplicateMenu:"=",removeMenu:"="},link:function(a){a.editItem=function(){window.location.href="/editmenu/"+a.menu.id},a.duplicateItem=function(b){noty({layout:"center",type:"confirm",text:d.getString("Are you sure you want to duplicate this menu? Note: You'll be redirect to edit the menu."),buttons:[{addClass:"alert tiny",text:d.getString("Yes, duplicate this menu!"),onClick:function(c){var g=angular.copy(b);g.promotions=[],f.start(),e.save(g).then(function(b){g.id=b.data.menuid,a.duplicateMenu(g),f.complete()},function(){f.complete(),noty({type:"error",layout:"topCenter",text:d.getString("Sorry, but there's been an error processing your request.")})}),c.close()}},{addClass:"secondary tiny",text:d.getString("No, go back."),onClick:function(a){a.close()}}]})},a.removeItem=function(b){var c=b.id;noty({layout:"center",type:"confirm",text:d.getString("Are you sure you want to delete this menu? Note: all menu data will be lost!"),buttons:[{addClass:"alert tiny",text:d.getString("Yes, delete this menu!"),onClick:function(g){f.start(),e.remove(c).then(function(){f.complete(),a.removeMenu(b)},function(){f.complete(),noty({type:"error",layout:"topCenter",text:d.getString("Sorry, but there's been an error processing your request.")})}),g.close()}},{addClass:"secondary tiny",text:d.getString("No, go back."),onClick:function(a){a.close()}}]})}}}}]);