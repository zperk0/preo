	...
	<tbody>
		<tr class="savedInput eventTR">
			<td class="eventTDName">
				...
				<input type="text" ng-click='expandOptions($event)' name="eName[{{ $index + 1 }}]" class="eventField noEnterSubmit" ng-model="event.name" placeholder="<?echo _("Click to add an event name");?>" pattern="^.{0,99}$" required readonly="readonly"/>
				<small class="error"><?echo _("Please type an event name (max 100chars)");?></small>
			</td>
			<td class="eventTDDesc">
				<input type="text" ng-click='expandOptions($event)' name="eDesc[{{ $index + 1 }}]" class="eventField noEnterSubmit" ng-model="event.description" placeholder="<?echo _("Click to add a description");?>" readonly="readonly" pattern="^.{0,250}$"/>
				<small class="error"><?echo _("Please type a description (max 250chars)");?></small>
			</td>
			<td class="eventTDDate">
				<input type="text" ng-click='expandOptions($event)' name="eDate[{{ $index + 1 }}]" class="eventField noEnterSubmit" ng-model='event.date' ng-value="formatDate(event.date)" pattern="^\d\d\/\d\d\/\d\d\d\d$" placeholder="<?echo _("DD/MM/YYYY");?>" required readonly="readonly"/>
				<small class="error"><?echo _("Date?");?></small>
			</td>
			<td class="eventTDTime">
				<input type="text" ng-click='expandOptions($event)' name="eTime[{{ $index + 1 }}]" class="eventField noEnterSubmit" ng-model='event.starttime' ng-value="event.starttime" pattern="\d\d:\d\d" placeholder="<?echo _("HH:MM");?>" required readonly="readonly"/>
				<small class="error priceError"><?echo _("Time?");?></small>
			</td>
			<td class="eventTDTime">
				<input type="text" ng-click='expandOptions($event)' name="eETime[{{ $index + 1 }}]" class="eventField noEnterSubmit" ng-model='event.endtime' ng-value="event.endtime" pattern="\d\d:\d\d" placeholder="<?echo _("HH:MM");?>" required readonly="readonly"/>
				<small class="error priceError"><?echo _("Time?");?></small>
			</td>
