  <form name="deliveryForm" id="deliveryForm" ng-submit="processForm()" novalidate>
    <div class='row deliveryHeader'> <h1 class="alignHeader" translate>Delivery Settings</h1>
    <div class='deliverySwitch'> <a href='#' translate>Enable delivery services</a>
  ...
  <div class='delivery-tab-content'>
      <div class='controlWrapper largeMarginBottom' ng-class="{'error': deliveryForm.deliveryZone.$invalid && triedSubmit }">
        <label class='colorLabel' for='deliveryZone' translate>Delivery Areas</label>
        <input type='text' id='deliveryZone' name='deliveryZone' ng-model="venueSettings.deliveryZone" ng-maxlength=200 placeholder='{{"eg. \"NW1, NW2...\"" | translate}}' ng-disabled="venue.deliverFlag==0"/>
        <small  ng-show="deliveryForm.deliveryZone.$invalid && triedSubmit" class="error" translate>Please type a delivery zone (max 200chars)</small>
      </div>
      <div class='controlWrapper largeMarginBottom' ng-class="{'error': deliveryForm.telephone.$invalid && triedSubmit }">
        <label class='colorLabel' for='telephone' translate>In case of customer queries contact:</label>
        ...
        <small  ng-show="deliveryForm.telephone.$invalid && triedSubmit" class="error" translate>Please enter a valid telephone number</small>
      </div>
      <div class='controlWrapper inlineControl' ng-class="{'error': deliveryForm.minValueOrder.$invalid && triedSubmit }">
        <label class='colorLabel inlineLabel' for='minValueOrder' translate>Minimum order value to qualify for delivery</label>
        ...
        <small  ng-show="deliveryForm.minValueOrder.$invalid && triedSubmit" class="error" translate>Please enter a valid number (e.g. 20.52)</small>
      </div>
      <div class='controlWrapper inlineControl' ng-class="{'error': deliveryForm.deliveryCharge.$invalid && triedSubmit }">
        <label class='colorLabel inlineLabel' for='deliveryCharge' translate>Delivery charge</label>
        ...
        <small  ng-show="deliveryForm.deliveryCharge.$invalid && triedSubmit" class="error" translate>Please enter a valid number (e.g. 3.25)</small>
      </div>
      <div class='controlWrapper inlineControl' ng-class="{'error': deliveryForm.deliveryBelow.$invalid && triedSubmit }">
        <label class='colorLabel inlineLabel' for='deliveryChargeBelow' translate>Free delivery for orders above</label>
        ...
        <small  ng-show="deliveryForm.deliveryChargeBelow.$invalid && triedSubmit" class="error" translate>Please enter a valid number (e.g. 3.25)</small>
      </div>
      <div class='controlWrapper inlineControl' ng-class="{'error': deliveryForm.deliveryLeadTime.$invalid && triedSubmit }">
      <label class='colorLabel inlineLabel' for='deliveryLeadTime' translate>Default delivery time (minutes) </label>
      ...
      <small  ng-show="deliveryForm.deliveryLeadTime.$invalid && triedSubmit" class="error" translate>Please enter a valid time interval in minutes (e.g. 20)</small>
      </div>
      <div class='controlWrapper inlineControl' ng-class="{'error': deliveryForm.deliveryDiscount.$invalid && triedSubmit && !isPosting }">
      <label class='colorLabel inlineLabel' for='deliveryDiscount' translate>Discount offered for delivery orders (%)</label>
      ...
      <small  ng-show="deliveryForm.deliveryDiscount.$invalid && triedSubmit && !isPosting" class="error" translate>Please provide a discount percentage (between 0 and 100)</small>
      </div>
