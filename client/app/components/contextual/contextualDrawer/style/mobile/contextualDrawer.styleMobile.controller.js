export default class contextualDrawerStyleController {
  ...
  constructor($q, $scope, $mdSidenav, Spinner, Snack, $stateParams, contextualDrawer, $location, $timeout, $sanitize,
   ...
   // Spinner.show('style-drawer-mobile');
    ...
    //   }
    //   $scope.$watch('drawerStyleMobileCtrl.StyleService.imagesModel',(nv)=>{
    //     this.model.images = nv;
    //   })
    //   $scope.$watch('drawerStyleMobileCtrl.StyleService.templatesModel',(nv)=>{
    //     this.model.templates = nv;
    //   })
    //   this._restoreImages();
       this.originalModel  = angular.copy(this.model);
    //   Spinner.hide('style-drawer-mobile');
    // }, ()=>{
    //   this._restoreImages()
    // }).catch((err)=>{
    //   console.log("err", err)
