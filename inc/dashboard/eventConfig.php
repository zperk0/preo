...
<div class="row">
	...
	<div class="large-12 columns">
		<form id="eventConfigForm" method="POST" data-abide>
			...
			<div class="row"> <!-- DUMMY -->
				<div class="large-12 columns">
					<table class="eventTable hide" id="event0">
						<tbody>
							<tr class="eventEdit eventTR">
								...
								<td class="eventTDTools">
									...
									<button type="button" class="eventTableButtons alert eventDelete" 		title="<?echo _("Delete");?>"						><i class="fi-x"></i></button>
								</td>
							</tr>
						</tbody>
					</table>
			...
			<div class="row"> 
				...
				<?if($_SESSION['event_edit_on']){
					...
					?>
						...
						<table class="eventTable" id="event<?echo ($eKey+1)?>">
							<tbody>
								<tr class="eventEdit eventTR">
									<td class="eventTDName">
										<input type="text" name="eName[<?echo ($eKey+1);?>]" class="eventField noEnterSubmit" value="<?echo $event['name'];?>" required/>
										<small class="error"><?echo _("Please type an event name");?></small>
									</td>
									<td class="eventTDDesc">
										<input type="text" name="eDesc[<?echo ($eKey+1);?>]" class="eventField noEnterSubmit" <?if(!empty($event['description'])){?>value="<?echo $event['description'];?>"<?}?> placeholder="<?echo _("Click to add a description");?>"/>
										<small class="error"><?echo _("Please type a description");?></small>
									</td>
									<td class="eventTDTime">
										<input type="text" name="eTime[<?echo ($eKey+1);?>]" class="eventField noEnterSubmit" value="<?echo date('H:i',strtotime($event['time']));?>" placeholder="<?echo _("HH:MM");?>" required/>
										<small class="error"><?echo _("Time?");?></small>
										</td>
									<td class="eventTDDate">
										<input type="text" name="eDate[<?echo ($eKey+1);?>]" class="eventField noEnterSubmit" value="<?echo date('d/m/Y',strtotime($event['date']));?>" placeholder="<?echo _("DD/MM/YYYY");?>" required/>
										<small class="error"><?echo _("Date?");?></small>
									</td>
									<td class="eventTDVisi">
										<div class="switch tiny"> 
